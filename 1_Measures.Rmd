...
```{r message=FALSE}
htt_data <- read.csv("scripts/claudia/HCT_Tracking.csv") |> 
  mutate(Participant = paste0("S", as.character(ID)),
         Condition = fct_recode(Condition, "NoGuessPerturbed" = "NoGuess_Perturbed")) |> 
...
# model <- rstanarm::stan_lmer(Distance ~ Condition + (1 + Condition | ID), data = hct_data, refresh=0, chains = 2)
# model <- glmmTMB::glmmTMB(Distance ~ Condition + (1 | ID), data = hct_data)
...
htt_scores <- htt_data |> 
  ...
  summarise(HTT_Mean = mean(Distance),
            HTT_n = n()) |> 
  ungroup() |> 
  complete(Participant, Condition, fill = list(HTT_Mean = NA, HTT_n = 0)) |> 
  tidyr::pivot_wider(names_from = "Condition", values_from = c("HTT_Mean", "HTT_n"))

df <- merge(df, htt_scores, by = "Participant")
```
...
df_physio |> 
  ...
  geom_rect(aes(xmin=1500, xmax=2500, ymin=-65, ymax=90), alpha=0.1, fill = "green") +
...
df <- df |> 
  ...
  group_by(Participant) |> 
  mutate(Outliers_RT = as.logical(performance::check_outliers(RT, method = "zscore_robust", threshold = qnorm(0.999))),
         Outliers_Physio = as.logical(performance::check_outliers(ECG_Rate_Mean_2.5s, method = "zscore_robust", threshold = qnorm(0.999))))
  ungroup()
```
...
df$ECG_Rate_Mean_2.5s[df$Outliers_Physio] <- NA
```
...
p2 <- df |> 
  ...
  facet_wrap(~Answer)
p3 <- df |> 
  ggplot(aes(x = ECG_Rate_Mean_2_3s, fill = Participant)) +
  geom_density(alpha = 0.1) +
  see::scale_fill_material_d(palette = "rainbow", guide = "none") +
  scale_x_continuous(expand=expansion(c(0, .05))) +
  scale_y_continuous(expand=expansion(c(0, .05))) +
