...
```{r message=FALSE, warning=FALSE}
model <- glmmTMB(RT ~ Answer * Phrasing + (1|Participant) + (1|Item), 
                 data = df) 
...
```{r message=FALSE, warning=FALSE}
model <- glmmTMB(Confidence ~ Answer * Phrasing + (1|Participant) + (1|Item), 
                 data = df, family = beta_family()) 
...
```{r message=FALSE, warning=FALSE}
model <- glmmTMB(HeartRate ~ Answer * Phrasing + (1|Participant) + (1|Item), 
                 data = df) 
...
- Higher heart rate in lies vs. truths, and higher heart rate in interrogation condition vs. polygraph.
- No interaction.
