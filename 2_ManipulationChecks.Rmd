...
```{r message=FALSE, warning=FALSE}
model <- glmmTMB(ECG_Rate_MeanRaw ~ Answer * Phrasing + (1|Participant) + (1|Item), data = df) 
...
```{r message=FALSE, warning=FALSE}
model <- glmmTMB(ECG_Rate_Mean ~ Confidence * Answer + (1|Participant) + (1|Item), 
                 data = df)
...
```{r message=FALSE, warning=FALSE}
estimate_relation(model, at = c("Confidence", "Answer")) |> 
  plot(length = 50, point = list(alpha = 0.3, size = 3.5)) 
```
...
```{r message=FALSE, warning=FALSE}
# ECG_Rate_Mean_2.5s
names(select(df, starts_with("ECG_")))
model <- glmmTMB(ECG_Rate_Mean_1.5_2.5s ~ Answer * Condition + (1|Participant) + (1|Item), 
                 data = dplyr::filter(df, abs(ECG_Rate_Mean_1.5_2.5s) < 10))
