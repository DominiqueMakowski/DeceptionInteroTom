...
results <- mydata %>% 
  ...
  rename(MAIA_noticing = MAIA1,
         MAIA_not_distracting = MAIA2,
         MAIA_not_worrying = MAIA3,
         MAIA_attention_regulation = MAIA4,
         MAIA_emotional_awareness = MAIA5,
         MAIA_self_regulation = MAIA6,
         MAIA_body_listening = MAIA7,
         MAIA_trusting = MAIA8) %>% 
...
write.csv(results, file = "liescale_processed.csv", row.names=FALSE)
```
...
data <- mydata %>% 
  ...
  mutate(DT_confidence = as.numeric(DT_confidence),
         ...
         ID = as.numeric(ID)) %>%
  mutate(DT_RT = ifelse(condition == "Social", social_stop - social_start,
...
results <- data %>%
  ...
  summarise(DT_RT_avg = mean(DT_RT, na.rm = TRUE),
            DT_confidence_avg = mean(DT_confidence, na.rm = TRUE))
...
data_combined <- read.csv("deceptiontask_extracted.csv") %>% 
  ...
  select(-c(contains(paste0("Q", 0:9)))) %>%
  left_join(read.csv("liescale_processed.csv")) %>% 
  select(-c(contains(paste0("Q", 0:9)))) %>%
