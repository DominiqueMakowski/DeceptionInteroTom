...
for (participant in list.files("../../data/data_experimental")) {
  df_sub <- read.csv(paste0("../../data/data_experimental/", participant, "/deception/", participant, "_deceptiontask.csv"), header = FALSE)[-c(1:11), -c(5:34, 37:46, 48:54)] %>% 
    mutate(DT_trial = c(1:80)) %>% 
    mutate(DT_condition = c(rep("Social", 40), rep("Polygraph", 40)))
  mydata <- rbind(mydata, df_sub)
}
...
data <- mydata %>% 
  ...
  mutate(DT_instruction = as.factor(DT_instruction)) %>% 
  mutate(ID = as.numeric(ID)) %>% 
  left_join(inclusion) %>% 
  
  select(ID, DT_trial, DT_condition, inclusion, inclusion2, everything()) #move columns to front

results <- data %>% 
  group_by(ID, DT_instruction, DT_style) %>% 
